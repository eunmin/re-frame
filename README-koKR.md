...
## re-frame에 관심을 가져야하는 이유

둘 중 하나:

1.  ClojureScript로 [SPA] 만들기 위한 프레임워크를 찾고 있거나,
2.  ReactJS가 2015년 초 Javascript 프레임워크 전쟁에서 이겼고 이것이 무엇을 의미하는지 대한 궁금해 할 것이다.
    `reactive programming`과 `functional programming`, `immutable data`의 조합은 **모든 것을 바꿔 놓은 것인가**?
    만약 그렇다면, 이런 패러다임을 가지는 언어를 어떻게 봐야할 것인가?

## re-frame

re-frame은 [Reagent]와 ClojureScript로 [SPAs]를 만들기 위한 패턴이다.

이 리파지토리에는 **이 패턴에 대한 설명**과 **구현에 대한 참고자료**가 있다.

McCoy의 말을 인용하면 : "짐, 이건 MVC지, 그런데 우린 그걸 모르고 있어."

re-frame 어플리케이션을 만드려면:
 - 데이터 구조를 설계한다. (data layer)
 - subscription를 만들고 등록한다. (query layer)
 - Reagent 컴포넌트 함수를 만든다. (view layer)
 - 이벤트 핸들러 함수를 만들고 등록한다. (control layer 또는 state transition layer)

특징:

1. 작성하는 함수는 순수해야한다. 따라서 어플리케이션에 계산 가능한 부분들에 대한 표현을 이해하고 독립적으로 테스트 할 수 있다.
   테스트를 위해 의존성 주입 같은 것은 필요하지 않다. 그래서 복잡한 부가적인 기능들이 많이 필요 없다.
2. 이 계산 가능한 부분들은 동적이고, 단방향의 Signal 그래프인 reactive data flow로 조합 할 수 있다.
3. 결과적인 구조는 reactive 루프인 두 스테이지안에 흐르는 "derived 데이터"를 포함한다.
   이 데이터가 실현되는 것을 빼면 명시적인 모델링 타임이 된다.   
   (원문 - 다시 번역 필요) The resulting architecture involves "derived data" flowing in a two-stage, reactive loop.
   Without realising it, you will be explicitly modelling time.
4. 빠르다. 이런 류의 고민(http://blog.scalyr.com/2013/10/angularjs-1200ms-to-35ms/)은 하지 않아도 된다.
5. 놀랄 만한 점은 re-frame이 정말 단순하다는 것이다. 참고용으로 작성된 구현은 ClojureScript로 200라인이 조금 넘는 정도다.
   오후 한나 절이면 배운다.
6. 단순하지만 확장성이 좋아 복잡한 어플리케이션을 잘 만들 수 있다. 프레임워크라는 것은 작은 규모에 사용하기에는 불필요한 오버헤드가 많다.
   - measure them instead by how they help you tame the complexity of bigger apps.
7. Re-frame은 주요 용어들을 지키고 있다: FRP-nature, unidirectional data flow, pristinely pure functions, conveyor belts,
   statechart-friendliness(FSM), immaculate hammock 개념.
   또 charming xkcd 자료를 준비 중(빠른 시일 내)이고 즐겁고 웃긴 T-shirt와 컨퍼런스(계획중)도 있다.
   잘 안될 이유가 있을까?
...
## 튜토리얼 목착
 ...
 - [데이터](#on-data)
 - [커다란 Ratom](#the-big-ratom)
 - [한 곳에 데이터가 있는 것에 대한 이점](#the-benefits-of-data-in-the-one-place)
 - [흐름](#flow)
 ...
 - [더 효과적인 Signal Graph](#a-more-efficient-signal-graph)
 - [두번째 흐름](#the-2nd-flow)
 - [이벤트 흐름](#event-flow)
 - [이벤트란 무엇인가?](#what-are-events)
 - [이벤트 디스패치](#dispatching-events)
 - [이벤트 핸들러](#event-handlers)
 - [라우팅](#routing)
 - [FSM을 통한 제어](#control-via-fsm)
 - [Reduce 처럼](#as-a-reduce)
 - [Derived Data, Everywhere, flowing](#derived-data-everywhere-flowing)
 - [로깅과 디버깅](#logging-and-debugging)
 - [서버와 통신하기](#talking-to-a-server)
 - [CPU Hog 문제](#the-cpu-hog-problem)
 - [요약](#in-summary)
